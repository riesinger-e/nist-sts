...
#[no_mangle]
pub extern "C" fn sts_RunnerTestArgs_new() -> Box<RunnerTestArgs> {
...
///   undefined behaviour.
#[no_mangle]
pub unsafe extern "C" fn sts_RunnerTestArgs_destroy(args: Option<Box<RunnerTestArgs>>) {
    _ = args;
}
...
setter! {
    ...
    fn sts_RunnerTestArgs_set_frequency_block(frequency_block: TestArgFrequencyBlock);
}

setter! {
    /// Set the argument for the Non-Overlapping Template Matching Test to the given value.
    fn sts_RunnerTestArgs_set_non_overlapping_template(non_overlapping_template: TestArgNonOverlappingTemplate);
}

setter! {
    /// Set the argument for the Overlapping Template Matching Test to the given value.
    fn sts_RunnerTestArgs_set_overlapping_template(overlapping_template: TestArgOverlappingTemplate);
}

setter! {
    /// Set the argument for the Linear Complexity Test to the given value.
    fn sts_RunnerTestArgs_set_linear_complexity(linear_complexity: TestArgLinearComplexity);
}

setter! {
    /// Set the argument for the Serial Test to the given value.
    fn sts_RunnerTestArgs_set_serial(serial: TestArgSerial);
}

setter! {
    /// Set the argument for the Approximate Entropy Test to the given value.
    fn sts_RunnerTestArgs_set_approximate_entropy(approximate_entropy: TestArgApproximateEntropy);
}
