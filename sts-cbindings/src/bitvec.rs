...
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_from_str(ptr: *const c_char) -> Box<BitVec> {
...
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_from_str_with_max_length(
    ptr: *const c_char,
    max_length: usize,
) -> Box<BitVec> {
...
/// * The de-allocation of the returned [BitVec] must be done via [sts_BitVec_destroy].
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_from_bytes(ptr: *const u8, len: usize) -> Box<BitVec> {
...
/// * The de-allocation of the returned [BitVec] must be done via [sts_BitVec_destroy].
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_from_bits(ptr: *const bool, len: usize) -> Box<BitVec> {
    // SAFETY: caller has to ensure that ptr is valid for reads up to len bytes / elements.
    let slice = unsafe { &*slice_from_raw_parts(ptr, len) };
...
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_clone(bitvec: &BitVec) -> Box<BitVec> {
...
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_destroy(bitvec: Option<Box<BitVec>>) {
...
/// * `bitvec` may not be mutated for the duration of this call.
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_len_bit(bitvec: &BitVec) -> usize {
    bitvec.0.len_bit()
}
...
#[no_mangle]
pub unsafe extern "C" fn sts_BitVec_crop(bitvec: &mut BitVec, new_bit_len: usize) {
