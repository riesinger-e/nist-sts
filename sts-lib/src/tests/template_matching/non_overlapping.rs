...
impl<'a> NonOverlappingTemplateTestArgs<'a> {
    pub fn new_with_custom_template(templates: TemplateArg<'a>, count_blocks: usize) -> Option<Self> {
        if (1..100).contains(&count_blocks) {
            Some(Self {
                templates,
                count_blocks,
    ...
    let count_matches_per_chunk_per_template = (0..count_blocks)
        .map(|block_idx| {
            ...
            let (base_mask, base_mask_last_bit_index) =
                (super::create_mask(template_len), template_len % BYTE_SIZE);
            ...
            let matches_per_template = base_templates
                ...
                .map(|&base_template| {
                    ...
                    let (mut mask, mut mask_last_bit_index) = {
                        ...
                        let last_bit_index =
                            super::right_shift_byte_vec(&mut mask, base_mask_last_bit_index, start_bit)
                                .unwrap();
                        (mask, last_bit_index)
                    };
                    ...
                    let (mut template, mut template_last_bit_index) = {
                        ...
                        let last_bit_index = super::right_shift_byte_vec(
                            &mut template,
                    ...
                    while i < max_shifts {
                        ...
                        let matched = (0..mask.len()).all(|idx| {
                            let byte = super::get_byte(&data.data, &last_byte, current_byte + idx);
                        ...
                        if (i % BYTE_SIZE + shift + start_bit) / BYTE_SIZE == 0 {
                            ...
                            mask_last_bit_index =
                                super::right_shift_byte_vec(&mut mask, mask_last_bit_index, shift)
                                    .unwrap();
                            template_last_bit_index =
                                super::right_shift_byte_vec(&mut template, template_last_bit_index, shift)
                                    .unwrap();
                        } else {
                            ...
                            mask_last_bit_index =
                                super::right_shift_byte_vec(&mut mask, base_mask_last_bit_index, shift)
                                    .unwrap();

                            template = Vec::from(base_template);
                            template_last_bit_index = super::right_shift_byte_vec(
                                &mut template,
                                base_template_last_bit_index,
                                shift,
                            )
