...
fn cmp_f64(got: f64, expected: f64) {
    assert!(f64::abs(got - expected) < f64::EPSILON, "Expected {expected}, got {got}")
}

/// Test the frequency test (No. 1)
#[test]
fn test_frequency_test() {
    let input = BitVec::from([true, false, true, true, false, true, false, true, false, true].as_slice());
